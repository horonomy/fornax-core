...
- `docs/adr/0024-telemetry-and-usage-analytics-stance.md` (FORNX-366, GA
  prep): consolidates the already-implemented "no product-usage telemetry
  by default" stance (`crates/fornax-types/src/privacy.rs`'s three opt-in
  gates, `docs/privacy-redaction-policy.md`) into one canonical, GA-facing
  document. Introduces no new data collection. Explicitly distinguishes
  itself from FORNX-327's website-only GA4 analytics.
